...
class GaussianSMAGP(GPModel):
    ...
    def update_W(self):
        ...
        return logW1, logW2, idx, W1_idx, W2_idx
    ...
    @autoflow((settings.float_type, [None, None]), (settings.float_type, [None, None]), (tf.int32, [None]), (tf.int32, [None]))
    @params_as_tensors
    def update_W1(self, X, Y, W1_idx, W2_idx):
        Kdiag = self.kern.Kdiag(X, full_output_cov=False)
        Kux = features.Kuf(self.feature, self.kern, X)
        Kuu = features.Kuu(self.feature, self.kern, jitter=settings.jitter)
    ...
    @autoflow((settings.float_type, [None, None]), (settings.float_type, [None, None]), (tf.int32, [None]), (tf.int32, [None]))
    @params_as_tensors
    def update_W2(self, X, Y, W1_idx, W2_idx):
        Kdiag = self.kern.Kdiag(X, full_output_cov=False)
        Kux = features.Kuf(self.feature, self.kern, X)
        Kuu = features.Kuu(self.feature, self.kern, jitter=settings.jitter)
        ...
        # compute new W2
        W1 = tf.gather(normalize(self.W1), W1_idx)
        logW2 = tf.reduce_sum(
            logW * tf.transpose(W1)[:, None, :],
            axis=0)
        # logW2 = logW2 - tf.reduce_logsumexp(logW2, axis=0, keepdims=True)
    ...
    def _build_likelihood(self):
       ...
       # compute KL
        ...
        if self.likelihood.__class__.variational_expectations \
            ...
            var_exp = self.likelihood.variational_expectations(fmean, fvar, tf.tile(Y, [self.K1 * self.K2, 1]))
        else:
